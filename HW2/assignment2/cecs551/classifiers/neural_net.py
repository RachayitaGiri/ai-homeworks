...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    temp1 = -scores[np.arange(N), y]
    temp2 = np.log(self.softmax(scores))
    ...
    #num = np.exp(scores[np.arange(N), y])
    scores = np.exp(scores)
    scores_sums = np.sum(scores, axis=1)
    cors = scores[np.arange(N), y]
    s = np.divide(scores, scores_sums.reshape((N, 1)))
    s[np.arange(N), y] = -(scores_sums - cors) / scores_sums
    s /= N
    dW2 = h1.T @ s
    db2 = np.sum(s, axis=0)
    hidden = s @ W2.T
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
      # get the sampled indices, to be used to retrieve the sample data
  ...
  def predict(self, X):
    ...
    out = np.dot(np.maximum(0, X @ W1 + b1), (W2 + b2))
